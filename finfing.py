...
from ..CPCA import CPCA_Block
from ..head3 import DemtHead
from ..TAFFC import TEXTURE_FFC
from ..TEXTURE_ffc import TEXTURE_FFC_Layer
def weight_init(module):
    for n, m in module.named_children():
        ...
        nn.ReLU, nn.GELU,nn.Sigmoid, nn.Softmax, nn.PReLU, nn.AdaptiveAvgPool2d, nn.AdaptiveMaxPool2d, nn.AdaptiveAvgPool1d,
        nn.Sigmoid, nn.Identity,nn.Dropout2d)):
            pass
...
class My_MHSA(nn.Module):
    def __init__(self, dim, num_heads, bias, mode):
        super(My_MHSA, self).__init__()
        self.num_heads = num_heads
        self.temperature = nn.Parameter(torch.ones(num_heads, 1, 1))
    ...
    def initialize(self):
        weight_init(self)
class My_Cross_Attention(nn.Module):
    def __init__(self, dim, num_heads, bias, mode):
        super(My_Cross_Attention, self).__init__()
        self.num_heads = num_heads
        self.temperature = nn.Parameter(torch.ones(num_heads, 1, 1))
    ...
    def forward(self, x,xf, mask=None):
        b, c, h, w = x.shape
        q = self.qkv1conv(self.qkv_0(xf))
        k = self.qkv2conv(self.qkv_1(xf))
        v = self.qkv3conv(self.qkv_2(x))
        if mask is not None:
            q = q * mask
            k = k * mask
...
class MSA_head(nn.Module):
    def __init__(self, mode='dilation', dim=128, num_heads=8, ffn_expansion_factor=4, bias=False,
                 LayerNorm_type='WithBias',AT = 1):
        super(MSA_head, self).__init__()
        self.norm1 = LayerNorm(dim, LayerNorm_type)
        if AT == 1:
            self.attn = Attention(dim, num_heads, bias, mode)
        elif AT ==2:
            self.attn = My_MHSA(dim, num_heads, bias, mode)

        elif AT ==3:
            self.attn = My_Cross_Attention(dim, num_heads, bias, mode)
...
class MCA_head(nn.Module):
    def __init__(self, mode='dilation', dim=128, num_heads=8, ffn_expansion_factor=4, bias=False,
                 LayerNorm_type='WithBias'):
        super(MCA_head, self).__init__()
        self.norm1 = LayerNorm(dim, LayerNorm_type)
    ...
    def forward(self, x,xf, mask=None):
        x = x + self.attn(self.norm1(x),xf, mask)
        x = x + self.ffn(self.norm2(x))
        return x

    def initialize(self):
        weight_init(self)
class FINDING_CLUE (nn.Module):
    def __init__(self, dim=128):
        super(FINDING_CLUE, self).__init__()
        self.FIND_BFA = MCA_head()
        self.F_TA = DemtHead()
        # self.F_TA = MSA_head(AT=2)
        self.TA = MSA_head(AT=1)
    ...
    def forward(self, x, side_x, mask):
        ...
        xF_B = self.FIND_BFA(x,xf,mask_d)


        x_ALL = self.TA(x)
        x = torch.cat((xF_B, x_ALL), 1)
        x = x.view(N, 3 * C, H, W)
        x = self.Fuse(x)
...
class Decoder(nn.Module):
    def __init__(self, channels=128):
        ...
        #
        # xiongrong1
        # self.texture_aware1 = TEXTURE_FFC(512)
        # self.texture_aware2 = TEXTURE_FFC(320)
        # self.texture_aware3 = TEXTURE_FFC(128)
        # self.texture_aware4 = TEXTURE_FFC(64)


        # xiongrong2
        # self.texture_aware1 = TEXTURE_FFC_Layer(512)
        # self.texture_aware2 = TEXTURE_FFC_Layer(320)
        # self.texture_aware3 = TEXTURE_FFC_Layer(128)
        # self.texture_aware4 = TEXTURE_FFC_Layer(64)
        ...
        # self.MSA5=FINDING_CLUE(dim = channels)
        # self.MSA4=FINDING_CLUE(dim = channels)
        # self.MSA3=FINDING_CLUE(dim = channels)
        # self.MSA2=FINDING_CLUE(dim = channels)
        ...
        # xiongrong4
        # self.pre_predtrans1  = TEXTURE_FFC_Layer(channels)
        # self.pre_predtrans2  = TEXTURE_FFC_Layer(channels)
        # self.pre_predtrans3  = TEXTURE_FFC_Layer(channels)
        # self.pre_predtrans4  = TEXTURE_FFC_Layer(channels)
        # self.pre_predtrans5  = TEXTURE_FFC_Layer(channels)

        # xiongrong3
        # self.pre_predtrans  = TEXTURE_FFC_Layer(channels)
        # xiongrong5
        # self.texture_aware  = TEXTURE_FFC_Layer(channels)


        # self.cpca  = CPCA_Block(channels, channels)
    ...
    # def forward(self, HP,E4, E3, E2, E1,shape):
    #     E4, E3, E2, E1= self.texture_aware1(E4), self.texture_aware2(E3), self.texture_aware3(E2), self.texture_aware4(E1)
        E4, E3, E2, E1= self.side_conv1(E4), self.side_conv2(E3), self.side_conv3(E2), self.side_conv4(E1)

        # E4, E3, E2, E1 = self.texture_aware(E4), self.texture_aware(E3), self.texture_aware(E2), self.texture_aware(E1)
...
if __name__=='__main__':
    ...
    E5 = torch.rand(2,128,24,24)
    E4 = torch.rand(2,128,24,24)
    P5 = torch.rand(2,1,24,24)
    mask_d = torch.sigmoid(P5)

    # net = MSA_module(E5, E4, P5)
    net = Decoder()
